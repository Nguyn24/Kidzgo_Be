...
-------------------------------------
 Các vấn đề thêm:
 2. Giải thích EntryType và isClassStarted
EntryType - ý nghĩa và luồng:
EntryType	Ý nghĩa	Khi nào dùng	Tự động hóa
immediate	Vào học ngay	Học viên đăng ký và có lớp đang tuyển sinh hoặc sắp khai giảng	Tạo Enrollment + Set Status = Studying
makeup	Vào học sau khi học bổ	Học viên nhập học giữa chừng (lớp đã Active) và cần tham gia buổi học bổ trước	Tạo Enrollment + Set Status = ClassAssigned
wait	Vào hàng chờ	Không có lớp phù hợp, chờ admin xếp lớp thủ công sau	KHÔNG tạo Enrollment + Set Status = WaitingForClass
Ví dụ cụ thể:
Case A: immediate
Lớp A đang tuyển sinh (Recruiting), chưa khai giảng
Gọi assign-class với entryType=immediate → Học viên vào học ngay từ đầu
Case B: makeup
Lớp B đã khai giảng và đang học (Active), đã qua 5 buổi
Học viên mới đăng ký muốn nhập học
Gọi assign-class với entryType=makeup → Học viên được xếp vào lớp nhưng cần học bổ các buổi đã qua trước khi học cùng lớp
Hệ thống sẽ tính UsedSessions = 0 (chưa học buổi nào), nhưng có thể cần tạo MakeupCredit hoặc đánh dấu cần học bổ
Case C: wait
Không có lớp phù hợp (hoặc lớp đã full)
Gọi assign-class với entryType=wait (không cần classId)
Registration chuyển sang status WaitingForClass
Admin sẽ xếp lớp thủ công sau khi có lớp mới
isClassStarted:
isClassStarted = true → Lớp đã bắt đầu (StartDate ≤ hiện tại)
isClassStarted = false → Lớp chưa bắt đầu
Tác dụng: Dùng để hiển thị cảnh báo cho admin biết học viên sẽ nhập học giữa chừng (cần học bổ).
3. Vấn đề cần cải thiện thêm
Hiện tại makeup chỉ set status = ClassAssigned nhưng chưa có logic tự động tạo MakeupCredit cho các buổi đã qua. Bạn có muốn tôi bổ sung logic này không?
