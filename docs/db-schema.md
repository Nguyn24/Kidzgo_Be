...
- `class_id`: Lớp khởi nguồn (lớp đang học khi tạo yêu cầu)
...
**Quy tắc**:
- Chỉ cho phép tạo khi học sinh đang có ghi danh ACTIVE.
- Không cho tạo trùng khi đã có request PENDING hoặc APPROVED cho cùng học sinh + lớp.
...
- `class_id` (uuid, FK → classes.id): Lớp đang học (khởi nguồn)
