...
### 4.2. `pause_enrollment_requests` - Yêu cầu bảo lưu

**Mục đích**: Lưu yêu cầu bảo lưu (tạm dừng học) của học sinh theo khoảng thời gian. Khi được APPROVED, hệ thống tự động tạm dừng ghi danh (class_enrollments.status = PAUSED) cho các lớp có buổi học nằm trong khoảng bảo lưu.

**Các attribute quan trọng**:
- `student_profile_id`: Học sinh xin bảo lưu
- `class_id`: (Optional) Lớp khởi nguồn nếu có; hệ thống có thể tự quét các lớp trong khoảng bảo lưu
- `pause_from` / `pause_to`: Khoảng thời gian bảo lưu
- `reason`: Lý do bảo lưu (optional)
- `status`: PENDING → APPROVED/REJECTED/CANCELLED
- `requested_at`: Thời gian tạo yêu cầu
- `approved_by` / `approved_at`: Người duyệt + thời gian duyệt
- `cancelled_by` / `cancelled_at`: Người hủy + thời gian hủy (chỉ hủy khi PENDING và trước `pause_from`)
- `outcome`: Kết quả sau bảo lưu (ContinueSameClass / ReassignEquivalentClass / ContinueWithTutoring)
- `outcome_note`: Ghi chú kết quả (optional)
- `outcome_by` / `outcome_at`: Người cập nhật kết quả + thời gian

**Quy tắc**:
- Chỉ cho phép tạo khi học sinh đang có ghi danh ACTIVE và có buổi học trong khoảng bảo lưu.
- Không cho tạo trùng khi đã có request PENDING hoặc APPROVED bị chồng lấp thời gian với cùng học sinh.
- Parent chỉ được hủy khi `status = PENDING` và thời điểm hiện tại < `pause_from`.
- Khi APPROVED: tự động tạm dừng các ghi danh ACTIVE có buổi học nằm trong khoảng bảo lưu, đồng thời tạo lịch sử.

### 4.3. `pause_enrollment_request_histories` - Lịch sử bảo lưu

**Mục đích**: Lưu lịch sử thay đổi ghi danh do bảo lưu (ai thay đổi, lúc nào, thay đổi gì).

**Các attribute quan trọng**:
- `pause_enrollment_request_id`: Yêu cầu bảo lưu nguồn
- `student_profile_id`: Học sinh
- `class_id`: Lớp bị ảnh hưởng
- `enrollment_id`: Ghi danh bị thay đổi
- `previous_status` / `new_status`: Trạng thái ghi danh trước/sau (ACTIVE → PAUSED)
- `pause_from` / `pause_to`: Khoảng bảo lưu áp dụng
- `changed_by` / `changed_at`: Người thao tác + thời gian

### 4.4. `attendances` - Điểm danh buổi học
...
### 4.5. `makeup_credits` - Credit học bù
...
### 4.6. `makeup_allocations` - Phân bổ credit vào buổi bù
...
```
1. pause_enrollment_requests (yêu cầu bảo lưu)
   ↓ (nếu APPROVED)
2. class_enrollments (đổi status sang PAUSED)
   ↓
3. pause_enrollment_request_histories (ghi lịch sử thay đổi)
```
...
#### `pause_enrollment_requests` - Yêu cầu bảo lưu
- `id` (uuid, PK): Định danh yêu cầu bảo lưu
- `student_profile_id` (uuid, FK → profiles.id): Học sinh xin bảo lưu
- `class_id` (uuid, FK → classes.id, nullable): Lớp khởi nguồn (optional)
- `pause_from` (date): Ngày bắt đầu bảo lưu
- `pause_to` (date): Ngày kết thúc bảo lưu
- `reason` (text, nullable): Lý do bảo lưu
- `status` (varchar(20)): Trạng thái - PENDING/APPROVED/REJECTED/CANCELLED
- `requested_at` (timestamptz): Thời gian tạo yêu cầu
- `approved_by` (uuid, FK → users.id, nullable): Người duyệt (role=STAFF/ADMIN)
- `approved_at` (timestamptz, nullable): Thời gian duyệt
- `cancelled_by` (uuid, FK → users.id, nullable): Người hủy (role=PARENT/STAFF)
- `cancelled_at` (timestamptz, nullable): Thời gian hủy
- `outcome` (varchar(30), nullable): Kết quả sau bảo lưu - ContinueSameClass/ReassignEquivalentClass/ContinueWithTutoring
- `outcome_note` (text, nullable): Ghi chú kết quả
- `outcome_by` (uuid, FK → users.id, nullable): Người cập nhật kết quả
- `outcome_at` (timestamptz, nullable): Thời gian cập nhật kết quả

#### `pause_enrollment_request_histories` - Lịch sử bảo lưu
- `id` (uuid, PK): Định danh lịch sử
- `pause_enrollment_request_id` (uuid, FK → pause_enrollment_requests.id): Yêu cầu bảo lưu nguồn
- `student_profile_id` (uuid, FK → profiles.id): Học sinh
- `class_id` (uuid, FK → classes.id): Lớp bị ảnh hưởng
- `enrollment_id` (uuid, FK → class_enrollments.id): Ghi danh bị thay đổi
- `previous_status` (varchar(20)): Trạng thái trước (ví dụ: ACTIVE)
- `new_status` (varchar(20)): Trạng thái sau (ví dụ: PAUSED)
- `pause_from` (date): Ngày bắt đầu bảo lưu
- `pause_to` (date): Ngày kết thúc bảo lưu
- `changed_by` (uuid, FK → users.id, nullable): Người thao tác
- `changed_at` (timestamptz): Thời gian thao tác
...
- Bảo lưu: tạo `pause_enrollment_requests`; khi APPROVED thì tạm dừng `class_enrollments` trong khoảng bảo lưu và ghi `pause_enrollment_request_histories`; kết quả sau bảo lưu lưu ở `outcome`.
